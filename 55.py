...
- JSON Login support (import session without OTP) - PER DEVICE
- Correct swap endpoint: /v1/rewardswap/fk/swap
"""
...
class GosatsClient:
    ...
    def validate_pan(self, pan: str) -> Optional[Dict]:
        ...
        device_info = {
            "device": {
                "platform": "android",
                "osVersion": 34,
                "deviceId": "b014b986c6b47dc9",
                "deviceName": "Shub's A06",
                "brand": "samsung",
                "model": "SM-A065F",
                "memory": 3836452864,
                "macAddress": "",
                "manufacturer": "samsung",
                "ipAddress": "10.18.179.92",
                "referer": "utm_source=google-play&utm_medium=organic",
                "versionCode": "3.0.4"
            },
            "advertisingId": "76f56849-f73a-43be-ad76-1425e85b6b3f"
        }
    ...
    def activate_supercoins(self) -> bool:
        data = self._request("POST", "/v1/rewardswap/fk/activate")
        return data is not None

    def get_balance(self) -> int:
        data = self._request("GET", "/v1/rewardswap/fk/config")
        if data:
            sc = data.get("data", {}).get("swapConfig", {}).get("supercoin", {})
            return sc.get("balance", 0)
        return 0

    def get_swap_rate(self) -> Optional[dict]:
        config = self._request("GET", "/v1/rewardswap/fk/config")
        if not config:
            return None
        
        sc = config.get("data", {}).get("swapConfig", {}).get("supercoin", {})
        swap_opts = sc.get("swapOptions", [])
        
        sats_opt = next((o for o in swap_opts if o.get("name") == "sats"), None)
        if not sats_opt:
            return None
        
        rate = sc.get("satsToFactor", 15)
        try:
            rate = int(rate) if str(rate).isdigit() else 15
        except:
            rate = 15
        
        return {
            "rate": rate,
            "min": sats_opt.get("min", 1),
            "max": sats_opt.get("max"),
            "fee": sats_opt.get("fee", 0)
        }

    def swap_coins(self, amount: int) -> bool:
        """Swap SuperCoins to Sats - using the correct endpoint format"""
        config = self._request("GET", "/v1/rewardswap/fk/config")
        if not config:
        ...
        sats_opt = next((o for o in swap_opts if o.get("name") == "sats"), None)
        if not sats_opt:
            log.error("Sats swap option not found")
            return False
        ...
        swap_inr = amount  # IMPORTANT: swapINR should be the amount of SuperCoins
        ...
        log.info(f"Swapping {amount} SC to {to_amount} Sats...")
        data = self._request("POST", "/v1/rewardswap/fk/swap", body)
        
        if data:
            if data.get("success") or data.get("data"):
                log.ok(f"Swap successful: {amount} SC → {to_amount} Sats")
                return True
            else:
                log.error(f"Swap failed: {data}")
                return False
...
class AsyncShopsyClient:
    ...
    # ============================================================
    # JSON LOGIN - IMPORT SESSION DIRECTLY
    # ============================================================
    ...
    # ============================================================
    # OTP LOGIN (Legacy - still supported)
    # ============================================================
...
async def process_one(phone: str, device: str, panel: dict, idx: int, total: int, is_single: bool = False, json_data: dict = None):
    ...
    try:
        async with client:
            ...
            # Try JSON import first (if json_data provided)
            if json_data:
                if client.import_session(json_data):
                    ...
                    session_file = DATA_DIR / f"{device_key}_session.json"
                    with open(session_file, 'w') as f:
                        json.dump(json_data, f)
            
            # Try loading from file if no JSON data provided
            if not imported:
                session_file = DATA_DIR / f"{device_key}_session.json"
                if session_file.exists():
                    try:
                        with open(session_file, 'r') as f:
                            tokens = json.load(f)
                        if client.import_session(tokens):
                            imported = True
                            await emit_to_all('status', {'device': device_key, 'status': 'login_success', 'message': 'Session loaded from file'})
                            log.ok(f"Session loaded from file for {device_key}")
                    except Exception as e:
                        log.warn(f"Failed to load session file: {e}")
            
            # Fallback to OTP login if JSON import failed
            if not imported:
                ...
                with open(session_file, 'w') as f:
                    json.dump(tokens, f)
                log.ok(f"Session saved to {session_file}")
            ...
            if not gosats or not gosats.logged_in:
                ...
                if is_new:
                    ...
                    config = gosats._request("GET", "/v1/rewardswap/fk/config")
            ...
            balance = gosats.get_balance()
            if balance > 0:
                await emit_to_all('status', {'device': device_key, 'status': 'swapping', 'message': f'Swapping {balance} SC...'})
