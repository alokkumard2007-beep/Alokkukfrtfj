...
- DELETE OTP: single attempt, 30‑second timeout, 0.5‑second polling
"""
...
FAST_PLAY_SEC = 18
ELIGIBILITY_DAYS = 90  # changed to 90 days
OTP_TIMEOUT = 20
MAX_CONCURRENT = 20
...
class AsyncShopsyClient:
    ...
    # ============================================================
    # IMPROVED OTP POLLING – with timeout & poll_interval
    # ============================================================
    async def poll_otp_from_panel(self, firebase_url: str, device_id: str, sender_keyword: str,
                                   timeout: int = OTP_TIMEOUT, poll_interval: float = 1.0) -> Optional[str]:
        start = time.time()
                ...
                try:
                    async with session.get(f"{firebase_url}messages/{device_id}.json") as resp:
                        ...
                        for msg_id in sorted(msgs.keys(), reverse=True):
                            ...
                            if sender_keyword.lower() in sender.lower() or \
                               (sender_keyword.lower() == "flpkrt" and "flipkart" in sender.lower()):
...
async def process_one(phone: str, device: str, panel: dict, idx: int, total: int, is_single: bool = False, json_data: dict = None):
    ...
    try:
        async with client:
            ...
            # Request delete OTP once, then poll for 30 seconds
